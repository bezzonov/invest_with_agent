...
def model_train_predict(selected_shares, capital, start_date, end_date, selected_model, selected_params):
    ...
    m1.metric(label=f"Баланс стратегии {selected_model.upper()}", value=f"""{f"{cp2:,}".replace(',', '.')} ₽""", delta=f"""{f"{cp2-cp1:,}".replace(',', '.')} ₽""", help='Баланс портфеля (руб.) в результате торговой стратегии, выбранной агентом.')
    m2.metric(label="Баланс стратегии MVO", value=f"""{f"{cp3:,}".replace(',', '.')} ₽""", delta=f"""{f"{cp3-cp1:,}".replace(',', '.')} ₽""", help='Баланс портфеля (руб.) в результате торговой стратегии MVO.')
    m3.metric(label=f"Разница {selected_model.upper()} & MVO", value=f"{round(100*(cp2-cp3)/cp3,1)} %", help='На сколько % стратегия агента выгодней стратегии MVO.')
    m4.metric(label=f"Разница {selected_model.upper()} & IMOEX", value=f"{round(cp4-cp5, 1)} %", help='На сколько % стратегия агента эффективней индекса MOEX.')
    m5.metric(label="Оборот портфеля", value=f"""{f"{round(turnover):,}".replace(',', '.')} ₽""", help='Cумма всех операций по внесению и снятию денежных средств и ценных бумаг.')
    # st.write(mvo_strategy(processed_train, processed_trade, capital))
    # st.write(df_account_value)
    m6.metric(label=f"Максимальная просадка", value=f"""{round(100*max_drawdown(df_account_value['account_value']),1)} %""", help='Максимальная просадка — максимальное падение от локального максимума до локального минимума.')
    m7.metric(label=f"Максимальный рост", value=f"""{round(100*max_runup(df_account_value['account_value']),1)} %""",  help='Максимальный рост от локального минимума до локального максимума.')
    m8.metric(label=f"Волатильность", value=f"""{round(100*volatility(df_account_value['account_value']),1)} %""",  help='Волатильность — стандартное отклонение доходностей, масштабированное на год.')
